// Temporary debug: Dump all guild data to the UI for troubleshooting
"use client";
...
export default function AdminGuildsClient({ guilds, error }: { guilds: any[]; error: string | null }) {
  return (
    <Section title="Admin Guilds">
      ...
      {!error && guilds.length > 0 && (
        <>
          <pre className="mb-4 bg-muted p-2 text-xs overflow-x-auto max-h-40">{JSON.stringify(guilds, null, 2)}</pre>
          <ul className="space-y-2">
            {guilds.map((g: any) => (
              <li key={g.guild_id} className="border rounded p-4 flex flex-col gap-1">
                ...
                {g.owner_name && (
                  <span style={{ background: '#fffae6', color: '#b45309', fontWeight: 'bold', padding: '2px 8px', borderRadius: 4, fontSize: 14, margin: '4px 0', display: 'inline-block' }}>
                    👑 Owner: {g.owner_name}
                  </span>
                )}
