...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-6 pr-8 shadow-xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border-primary/20 bg-primary/10 backdrop-blur-md text-foreground",
        success: "success group border-green-300/70 bg-green-50/80 text-green-900 dark:text-green-900",
        info: "info group border-blue-300/70 bg-blue-50/80 text-blue-900 dark:text-blue-900",
        warning: "warning group border-amber-300/70 bg-amber-50/80 text-amber-900 dark:text-amber-900",
        destructive:
          "destructive group border-red-400/70 bg-red-600/90 text-white",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, children, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ...
      {...props}
    >
      <span className="absolute left-0 top-0 bottom-0 w-1 bg-primary/80 group-[.success]:bg-green-600 group-[.info]:bg-blue-600 group-[.warning]:bg-amber-500 group-[.destructive]:bg-red-500" />
      {children}
    </ToastPrimitives.Root>
  )
})
