...
import ActionModal, { ModAction } from "@/app/guilds/[id]/moderation/components/action-modal";
import CasesList from "@/app/guilds/[id]/moderation/components/cases-list";
import ModerationSettings from "@/app/guilds/[id]/moderation/components/moderation-settings";
import CaseDetail from "@/app/guilds/[id]/moderation/components/case-detail";

// Fetch actual guild info
const getGuildInfo = async (guildId: string) => {
  try {
    const response = await fetch(`/api/guilds/${guildId}/info`);
    if (response.ok) {
...
export default function ModerationPanel({ guildId }: { guildId: string }) {
  ...
  useEffect(() => {
    ...
    const loadGuildInfo = async () => {
      try {
        ...
        console.error("Failed to fetch guild info:", error);
        setGuildInfo({ id: guildId, name: "Unknown Server", group: null });
      }

      // Fetch moderation stats
      await fetchStats(guildId);
    };
            ...
            <Card>
              <CardContent className="p-0">
                <div className="text-center py-16">
                  ...
                  <h4 className="text-lg font-semibold mb-2">No Case Selected</h4>
