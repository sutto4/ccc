...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      // Active servers (not left) - check if status column exists
    ...
    // For now, set totalUsers to 0 since member_count column might not exist
    // We can add this later when we have the actual member count data
    const totalUsers = 0;
    ...
    const stats = {
      ...
      averageUsersPerServer: totalServers > 0 ? Math.round(totalUsers / totalServers) : 0
    };
