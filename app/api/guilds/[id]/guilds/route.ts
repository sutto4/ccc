...
export const GET = withAuth(async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
	...
	if (base) {
		...
		const primaryUrl = hasApiSuffix
			? `${base}/guilds/${guildId}/guilds`
			: `${base}/api/guilds/${guildId}/guilds`;
		const fallbackUrl = hasApiSuffix
			? `${base.replace(/\/api$/i, "")}/api/guilds/${guildId}/guilds`
			: `${base}/guilds/${guildId}/guilds`;
	...
	// For now, return empty guilds array
	// This should be replaced with actual database logic to fetch guild information
	const guilds = [];
