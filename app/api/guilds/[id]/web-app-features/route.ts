  ...
  try {
    ...
    try {
      guildFeatures = await query(
        'SELECT gf.feature_name, gf.enabled, f.feature_name as display_name FROM guild_features gf LEFT JOIN features f ON gf.feature_key = f.feature_key WHERE gf.guild_id = ?',
        [guildId]
      );
